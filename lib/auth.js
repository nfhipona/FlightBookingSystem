...
exports.signout = (req, res) => { // always success - clear token when provided or just log off the user
    ...
    this.verify(token, (err, decoded) => {
        ...
        helper.send200(res, null, c.USER_SIGNED_OUT);
	});
}
...
exports.verify = (token, next) => {
    ...
    jwt.verify(token, jwtConf.secret, (err, decoded) => {
        ...
        if (decoded) {
            client.exists(decoded.id, (err, reply) => {
...
exports.verifyWithType = (type) => (req, res, next) => {
    ...
    const token = req.get('x-access-token');
    if (!token) { return helper.send403(res, new Error('Bad request.'), 'Invalid token'); }
    ...
    this.verify(token, (err, decoded) => {
        ...
        if (decoded && decoded.type == type) {
            ...
            if (decoded) {
                helper.send401(res, err, c.UNAUTHORIZED_REQUEST);
            }else{
                helper.send403(res, err, c.FORBIDDEN_REQUEST);
            }
        }
    });
}
