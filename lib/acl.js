...
module.exports = (database) => {
    ...
    const acl = (resource, mode, skip_maintenance_check = false) => (req, res, next) => {
        ...
        function _validate(conn) {
            ...
            const query = `SELECT ${fields} FROM resource_permission p \
    ...
    const login_check = (resource, mode) => (req, res, next) => {
        ...
        function _proceed() {
            ...
            // skip validation
            if (roleCode.isEqualToStr('sup_admin') || roleCode.isEqualToStr('sys_admin')) return helper.send200(res, data, c.USER_SIGNIN_SUCCESS);
